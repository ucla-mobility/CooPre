...
class DataAugmentor(object):
    ...
    def random_lidar_downsampling(self, data_dict=None, config=None):
        """
        Randomly downsamples LiDAR points to simulate sensor sparsity.

        Parameters
        ----------
        data_dict : dict
            Dictionary containing 'lidar_np' key with LiDAR points.
        config : dict
            Configuration dictionary with 'DOWNSAMPLE_RATIO' (float between 0 and 1).

        Returns
        -------
        dict
            Updated data_dict with downsampled LiDAR points.
        """
        if data_dict is None:
            return partial(self.random_lidar_downsampling, config=config)

        points = data_dict['lidar_np']
        downsample_ratio = config['DOWNSAMPLE_RATIO'][0]

        assert 0.0 < downsample_ratio <= 1.0, \
            "DOWNSAMPLE_RATIO must be a float between 0 and 1."
        ...
        data_dict['lidar_np'] = downsampled_points
        return data_dict
